...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function tokenOfOwner(address owner) public view virtual returns(uint256[] memory tokenids){
        uint256 length = balanceOf(owner);
        tokenids = new uint256[](length);
        for(uint256 index=0; index < length; index++){
            tokenids[index] = _ownedTokens[owner][index];
        }
...
contract WildCraft721Badge is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {

    uint256 private _nextTokenId = 1;

    mapping (address => bool) private operators;
    
    event Operator(address _user,bool isOperator);

    constructor()
        ERC721("WildCraft Badge NFT", "WCBN","https://wildcraft.fun/api/badge.json")
        Ownable(_msgSender())
    {}
